...
> [!CAUTION]
> **자의적인 Git 커밋 및 푸시 절대 금지**: 사용자의 명시적인 지시나 승인이 없는 경우, AI 에이전트는 자의적으로 `git commit`이나 `git push` 명령을 절대 수행하지 않습니다. 로컬에서 작업 및 검증까지만 완료하고 사용자에게 보고합니다.
...
- `.agents/docs/blog-architecture-analysis.md`: 상세 시스템 아키텍처 분석 및 Vercel 문제 해결 가이드 문서.
...
2. **자의적 Git 커밋/푸시 금지**: 사용자가 직접 요청하기 전까지 `git commit`이나 `git push`를 자의적으로 수행하지 않습니다.
3. **원본 콘텐츠 직접 수정 금지**: `content/` 폴더는 사용자가 옵시디언 앱에서 작성하여 Push하는 영역이므로 구조를 훼손하지 않습니다.
4. **안전한 파싱 (Safe Parsing) 준수**: 마크다운 파서 및 Frontmatter 파싱 시 에러 예방 가드를 적용하여 잘못된 마크다운으로 인해 빌드가 깨지지 않도록 합니다.
5. **Pure SSG 준수**: 런타임 서버리스 동적 파일 파싱 로직을 절대 도입하지 않습니다.
